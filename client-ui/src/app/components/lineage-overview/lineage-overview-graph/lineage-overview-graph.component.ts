...
export class LineageOverviewGraphComponent implements AfterViewInit, OnDestroy {

    @Input()
    embeddedMode: boolean;

    @ViewChild(CytoscapeNgLibComponent, { static: true })
    cytograph: CytoscapeNgLibComponent;
        ...
        this.subscriptions.push(this.store
            ...
            .subscribe(state => {
                if (state && this.cytograph.cy) {
                    const linageData = _.cloneDeep(state.graph.lineage);
                    this.cytograph.cy.add(linageData);
